...
/**
 * When an analysis run happens (SPEC 7.2).
 *
 * Classifier does what one run does and starts nothing by itself; this is what
 * starts it, along the three roads SPEC 7.2 names:
 *
 ...
 * twice.** Classifier declines a second start while it is busy, so the request
 ...
 * here. Twice cannot happen for the same reason: every run reads the unanalysed
 * notes back, and one that is through is not among them any more.
 *
...
public:
    /** `classifier` outlives the scheduler and is not owned by it. */
    explicit AnalysisScheduler(Classifier *classifier, QObject *parent = nullptr);
