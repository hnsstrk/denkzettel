    ...
    , m_focus(new KSvg::FrameSvg(this))
{
    ...
    for (KSvg::FrameSvg *frame : {m_field, m_focus}) {
    ...
    // theme, so they are set where the theme is read. applyHull() below calls
    // it, and it is the only place that has both halves.
    ...
    // Connected before the first reload() below, because that reload is what
    // builds the first image set and this window's two frames hang on it.
    connect(m_hull, &capture::WindowHull::changed, this, &CaptureWindow::applyHull);
...
void CaptureWindow::applyHull()
{
    // The two frames of this window hang on the hull's image set, and it is a
    // fresh one after every reload. Left out, they would keep drawing the old
    // theme's graphic on a window that has changed theme, and no return value
    // would say so (issue #100, F5; the focus layer beside it is the same trap
    // a fourth time, issue #102, AK 4).
    for (KSvg::FrameSvg *frame : {m_field, m_focus}) {
        frame->setImageSet(m_hull->imageSet());
    }
...
void CaptureWindow::applyTextColours()
{
    ...
    QColor noteColour = themeText.normal.isValid()
        ? m_hull->textColour()
...
void CaptureWindow::present()
{
    ...
    // notice missing — it shows only on the second opening. The effects come
    // with it and in the same breath, for a harder reason — the class comment
    // of bindToWindow() carries both.
    m_hull->bindToWindow();
}
