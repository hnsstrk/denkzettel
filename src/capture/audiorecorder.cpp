...
{
    // The stem is the note's, not the recording's — the full export of SPEC 8.3
    // writes its `.md` under the same one. noteFileStem() carries the rule and
    // the reason for the departure from `created_at`.
    return noteFileStem(createdAt) + QStringLiteral(".ogg");
}
