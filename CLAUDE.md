...
81. **`QTcpServer::newConnection` counts sockets, and Qt opens one that carries
...
82. **`msgcomm` counts an obsolete `#~` entry as present, so a merged catalogue
...
83. **A picture runner that resizes a page by a number of its own measures that
