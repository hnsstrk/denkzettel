                ...
                QMessageBox::ButtonRole answer)
{
    QTimer::singleShot(0, qApp, [&window, name, answer] {
        ...
        // Nachprüfung, Stelle 2: worauf zeigt die Liste, während gefragt wird?
        qInfo("  Auswahl der Liste während des Dialogs: Zeile %d", list->currentIndex().row());

        // Nachprüfung, Stelle 5: Symbole an den Knöpfen — Struktur, nicht Bild.
        for (QAbstractButton *button : buttons) {
            const QIcon icon = button->icon();
            qInfo("  Symbol an „%s“: gesetzt %d, Name „%s“, verfügbare Größen %lld",
                  qUtf8Printable(button->text()), icon.isNull() ? 0 : 1,
                  qUtf8Printable(icon.name()),
                  static_cast<long long>(icon.availableSizes().size()));
        }
        const QIcon fromTheme = QIcon::fromTheme(QStringLiteral("document-save"));
        const QIcon fromItem = KStandardGuiItem::save().icon();
        qInfo("  Im selben Prozess: Symbolthema „%s“ · fromTheme(document-save) null %d, Größen %lld · "
              "KStandardGuiItem::save().icon() null %d, Größen %lld · Stil „%s“, ButtonsHaveIcons %d",
              qUtf8Printable(QIcon::themeName()), fromTheme.isNull() ? 1 : 0,
              static_cast<long long>(fromTheme.availableSizes().size()), fromItem.isNull() ? 1 : 0,
...
void measureLayout(QWidget &window, const QString &state)
{
    ...
    QWidget *headStack = edit->parentWidget()->parentWidget();
    qInfo("Kopfzeile (%s): Knopf „Bearbeiten“ b=%d bei x=%d · „Löschen“ b=%d · Stapel b=%d, rechter Rand bei %d",
          qUtf8Printable(state), edit->width(), edit->mapTo(&window, QPoint(0, 0)).x(),
          buttonNamed(window, QStringLiteral("Löschen"))->width(), headStack->width(),
          headStack->mapTo(&window, QPoint(0, 0)).x() + headStack->width());
...
int main(int argc, char **argv)
{
    ...
    // 5 — Raumaufteilung im größeren Fenster.
    {
        ...
        shoot(window, QStringLiteral("20-lesen-1400x900.png"));
        buttonNamed(window, QStringLiteral("Bearbeiten"))->click();
        QTest::qWait(150);
    ...
    // Fenster, am Stapel selbst, nicht an seinen beiden Seiten.
    {
        ...
        // Und die Kopfzeile selbst, die den Sprung verursacht hatte.
        qInfo("Kopfzeile des Detailbereichs: Höhe im Bearbeiten-Zustand %d "
              "(Merkmalszeile + Fußzeile fangen den Höhenverlust unten auf: %d px weniger Text)",
              badge->parentWidget()->parentWidget()->height(), stackHeightReading - stack->height());
    ...
    // 9 — die Heilung von Befund 2 greift in den Auswahlwechsel ein. Also alle
    // drei Antworten daran gehalten, nicht nur die, die den Befund erzeugt hat.
    {
        for (const QMessageBox::ButtonRole answer :
             {QMessageBox::DestructiveRole, QMessageBox::AcceptRole}) {
            const QString label = answer == QMessageBox::DestructiveRole
                                      ? QStringLiteral("Verwerfen")
            ...
            withDialog(
                window, [list] { list->setCurrentIndex(list->model()->index(1, 0)); },
                QStringLiteral("18-auswahlwechsel-%1.png").arg(label.toLower()), answer);
            QTest::qWait(200);

            qInfo("Auswahlwechsel mit Antwort „%s“: Auswahl auf Zeile %d (angeklickt war 1), "
                  "Editor offen %d, Notiz im Speicher enthält „Vault“ %d",
                  qUtf8Printable(label), list->currentIndex().row(),
                  editorOf(window)->isVisible() ? 1 : 0,
                  store.note(edited)->content.contains(QStringLiteral("Vault")) ? 1 : 0);
            shoot(window, QStringLiteral("19-nach-auswahlwechsel-%1.png").arg(label.toLower()));
        }
    }
