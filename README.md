...
- For the transcription only, and only at runtime: `whisper-cpp` with a GGML
  ...
  **The model is fetched there too.** A size that is not on disk stands in the
  list as "medium — not downloaded" and choosing it asks once, naming the size
  of the file; the line under the list then carries the progress and the button
  beside it stops the download. It comes from the address the upstream
